...
const Quiz = () => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const response = await axios.get(`/api/quiz/${quizTempId}`,
      );
  ...
  const handleOptionClick = (optionId) => {
    ...
    const isSameResponse = allQuestions.some(
      (question) =>
        question.qId === questionData._id && question.optionId === optionId
    );

    const isResponseNotExist = !allQuestions.some(
      (question) => question.qId === questionData._id
    );
  ...
  const OptionButtonText = (option, index) => {
    return (
      <button
        ...
        style={{
          borderColor: `${userResponses.questions.some(
            (question) => question.optionId === option._id
          )
            ? "#5076FF"
            : ""
            }`,
