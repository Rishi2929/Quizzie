...
const Quiz = () => {
  const [quizData, setQuizData] = useState({}); //storing all quiz data fetching from database
  const [questionData, setQuestionData] = useState({}); //storing the question which will be shown on screen

  const [questionCounter, setQuestionCounter] = useState(0);  //keeping track of the question number
  ...
  //when we fetch quid data initially, we store id or each question and that question correctAnswer if in a object
  ...
  const fetchQuizData = async (quizId) => {
    try {
      ...
      if (response?.data?.success && response?.data?.quiz) {
        ...
        setCorrectAnswers([...quizResponse.questions.map(question => ({ qId: question._id, correctAnswer: question.correctAnswer }))]);
      }
  ...
  const handleOptionClick = (optionId) => {
    ...
    //ex: userResponses = [{qId: "", optionId: ""}, {}]
    let allQuestions = [...userResponses];
  ...
  const OptionButtonImg = (option, index) => {
    return (
      <button
        ...
        style={{ borderColor: `${userResponses?.some((question) => question.optionId === option._id) ? "#5076FF" : ""}`, }}
      >
        <img src={option?.imgUrl} alt="img url" className={styles["option-img"]} />
    ...
    return (
      <div className={styles["quiz-container"]}>
        <div className={styles["quiz-body"]}>
          ...
          {!isLoading && <h1>Quiz Data not found</h1>}
        </div>
      </div>
    );
  }

  if (isQuizCompleted && quizData?.quizType === "QA") {
    return (
      <QuizCompleted
  ...
  } else if (isQuizCompleted && quizData?.quizType === "Poll") {
    return (
        ...
        <div className={styles["quiz-body"]}>
          <>
            ...
            <div className={styles["question-body"]}>
              <div className={styles["question-title"]}>{questionData.questionTitle}</div>
